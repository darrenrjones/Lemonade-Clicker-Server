...
//   usaeusaebjsabjsaro4352: {
//     currectCahst: 3513532,
//     coosajtosaj: flase,
		
//   } 
}; //this is reset every time the server restarts
...
function updateDB() {
  for (var userId in activeUsers) {
    ...
    User.findByIdAndUpdate(userId, activeUsers[userId], {new:true})
    		.then(user => {
...
wss.on('connection', function connection(ws) {
  ws.on('message', function incoming(message) {
      ...
      case 'connect_user':
          ...
          activeUsers[msgJSON.userId] = getUserFromDb(msgJSON.userId); // User.find({userId})
